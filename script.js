...
function _haKeywordSottomano(lc) {
    return /\b(sottomano|promemoria|memo|appuntalo|appunto)\b/.test(lc);
}
function _haKeywordSpesa(lc) {
    return /\b(spesa|lista\s+spesa)\b/.test(lc);
}
...
function elaboraComandoVocale(testo) {
    ...
    if (matchCancella) {
        ...
        // Cancellazione in lista spesa o sottomano (sia se modal aperto, sia con keyword)
        if (contestoUI === 'spesa' || _haKeywordSpesa(lcRest)) {
            const q = _stripKeywords(rest, ['lista spesa', 'spesa', 'dalla', 'dal']).trim();
            _cancellaVoceLista('lista_spesa', q, 'LISTA SPESA');
            return;
        }
        if (contestoUI === 'sottomano' || _haKeywordSottomano(lcRest)) {
            const q = _stripKeywords(rest, ['sottomano', 'promemoria', 'memo', 'dal', 'dai']).trim();
            _cancellaVoceLista('sottomano', q, 'SOTTOMANO');
            return;
        }
        // Cancellazione in agenda
    ...
    // 2) MODAL APERTO → forza contesto (sovrascrive eventuali keyword)
    ...
    if (_haKeywordSpesa(lcOriginale)) {
        const cleaned = _stripKeywords(testo, ['lista spesa', 'spesa', 'aggiungi alla', 'aggiungi a', 'metti in', 'metti nella', 'metti nel']);
        if (cleaned.length >= 1) {
    ...
    if (_haKeywordSottomano(lcOriginale)) {
        const cleaned = _stripKeywords(testo, ['sottomano', 'promemoria', 'memo', 'appuntalo', 'appunto', 'aggiungi al', 'metti nel', 'metti in']);
        if (cleaned.length >= 1) {
    ...
    // 4) AGENDA: estrai data, orario, telefono
    ...
    // 5) Se il giorno è diverso da quello visualizzato, salva direttamente (no routing schemi)
    if (giornoTarget !== giornoCorrente) {
        _inserisciInRigaNormaleSuGiorno(giornoTarget, oraInfo.ora, testoBase, telInfo.telefono);
        return;
    }
    
    // 6) Stesso giorno: prova prima il routing schemi, poi riga normale
