...
// Festivi fissi italiani: data MM-DD → nome festività
const festiviFissi = { "01-01":"Capodanno", "01-06":"Epifania", "04-25":"Liberazione", "05-01":"Festa Lavoro", "06-02":"Festa Rep.", "08-15":"Ferragosto", "11-01":"Ognissanti", "12-08":"Immacolata", "12-25":"Natale", "12-26":"S. Stefano" };
// Pasqua e Pasquetta cambiano ogni anno (chiave YYYY-MM-DD)
const pasqua = { 
    "2026-04-05":"Pasqua", "2026-04-06":"Pasquetta",
    "2027-03-28":"Pasqua", "2027-03-29":"Pasquetta",
    "2028-04-16":"Pasqua", "2028-04-17":"Pasquetta",
    "2029-04-01":"Pasqua", "2029-04-02":"Pasquetta"
};
function festivoPerData(iso) {
    return pasqua[iso] || festiviFissi[iso.substring(5)] || null;
}
// Manteniamo l'alias vecchio per retro-compatibilità ma rimandiamolo alla nuova logica
const festivi2026 = new Proxy({}, { get: (_, mmdd) => festiviFissi[mmdd] || null });
    ...
    db.ref('notifiche_log').orderByChild('timestamp').limitToLast(50).on('value', (snapshot) => {
        ...
        Object.keys(logs).reverse().forEach(key => {
            ...
            item.onclick = () => {
                ...
                // Richiesta utente: dopo aver aperto la notifica, scrolla in cima alla
                // pagina (non sulla riga). Evidenziamo brevemente la riga corrispondente
                // così l'utente la individua subito quando scorre giù.
                setTimeout(() => {
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                    const rigaEl = document.getElementById('slot-' + n.rigaId);
                    if (rigaEl) { 
                        rigaEl.style.transition = 'background-color 0.6s';
...
function autoResize(el) { 
    if (!el) return;
    // iOS Safari ha problemi a calcolare scrollHeight subito dopo il render.
    // Triplo passaggio: subito, al frame successivo, e dopo 80ms (dopo il layout completo).
    const apply = () => {
...
// Ricalcola TUTTE le textarea visibili (utile dopo cambio orientamento, font load, ecc.)
function _ricalcolaTutteTextarea() {
    document.querySelectorAll('textarea').forEach(autoResize);
}
window.addEventListener('orientationchange', () => setTimeout(_ricalcolaTutteTextarea, 300));
window.addEventListener('resize', () => { clearTimeout(window._raTimer); window._raTimer = setTimeout(_ricalcolaTutteTextarea, 200); });
document.addEventListener('DOMContentLoaded', () => setTimeout(_ricalcolaTutteTextarea, 500));
if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(() => setTimeout(_ricalcolaTutteTextarea, 100));
}
...
function initCalendar() {
    ...
    if(!mp.options.length) {
        ...
        // Genera mesi da Gennaio 2026 a Dicembre 2029 (4 anni)
        for (let anno = 2026; anno <= 2029; anno++) {
            nomiMesi.forEach((m, i) => {
                mp.add(new Option(`${m} ${anno}`, `${anno}-${String(i+1).padStart(2,'0')}`));
            });
        }
        // Default: mese corrente se è nell'intervallo, altrimenti Gennaio 2026
        const oggi = new Date();
        const annoOggi = oggi.getFullYear();
        const meseOggi = String(oggi.getMonth()+1).padStart(2,'0');
        if (annoOggi >= 2026 && annoOggi <= 2029) {
            mp.value = `${annoOggi}-${meseOggi}`;
        } else {
            mp.value = '2026-01';
        }
    ...
    for(let d=1; d<=new Date(y, m, 0).getDate(); d++) {
        ...
        db.ref('agenda/'+iso).on('value', s => {
            ...
            Object.values(data).sort((a,b)=> cleanH(a.h)-cleanH(b.h)).forEach(v => {
                let testoBreve = v.isBattesimoBlock ? "BATTESIMO" : (v.isNozzeBlock ? "NOZZE" : (v.isWedBlock ? "MATRIMONIO" : (v.isWed ? "MATRIMONIO" : (v.t ? v.t.trim().split(/\s+/).slice(0, 2).join(' ').toUpperCase() : ""))));
...
function renderGiorno() {
    ...
    const haSchemaCaricato = Object.values(datiGiorno).some(it => it && (it.isWedBlock || it.isBattesimoBlock || it.isNozzeBlock));
...
let visualizzazione = {};
    ...
    Object.keys(datiGiorno).forEach(key => {
        ...
        // Per gli SCHEMI (matrimonio/battesimo) usiamo l'orario di inizio reale
        // come chiave di sort, così se ci sono più schemi nello stesso giorno
        // vengono ordinati cronologicamente.
        let sortKey;
        if (item.isWedBlock) {
            // Primo orario disponibile: sposo → sposa → chiesa → sala
            const wedH = cleanH(item.sposo_h) || cleanH(item.sposa_h) || cleanH(item.chiesa_h) || cleanH(item.sala_h) || 0;
            sortKey = wedH || 1; // fallback 1 = prima delle righe normali
        } else if (item.isBattesimoBlock) {
            const batH = cleanH(item.cerimonia_h) || cleanH(item.ricevimento_h) || 0;
            sortKey = batH || 1;
        } else if (item.isNozzeBlock) {
            const nozH = cleanH(item.cerimonia_h) || cleanH(item.ricevimento_h) || 0;
            sortKey = nozH || 1;
        } else {
            sortKey = cleanH(item.h)
    ...
    sorted.forEach((item) => {
        if(item.t || mostraTutteRighe || item.isAdmin || item.isBattesimoBlock || item.isNozzeBlock || item.isWedBlock || item.id.startsWith("ex") || item.id.startsWith("rep_")) {
...
if(item.isBattesimoBlock) {
    ...
    <button onclick="condividiSchema('${item.id}', 'battesimo')" data-testid="btn-condividi-bat" style="background:none; border:none; color:white; cursor:pointer; font-size:20px; width:30px; display:flex; justify-content:center;" aria-label="Condividi battesimo">📤</button>
    <input type="text" value="${item.titolo_bat || 'BATTESIMO'}" 
...
// === BLOCCO NOZZE (cloned from Battesimo, gold/bronze color) ===
if(item.isNozzeBlock) {
    const div = document.createElement('div'); div.className = "macro-nozze"; div.id = "slot-" + item.id;
    const GOLD = "#b8860b";
    div.style.border = "2px solid " + GOLD; 
    div.innerHTML = `<div class="titolo-nozze" style="background:${GOLD}; color:white; padding:10px; display:flex; align-items:center; justify-content:space-between;">
    <button onclick="condividiSchema('${item.id}', 'nozze')" data-testid="btn-condividi-noz" style="background:none; border:none; color:white; cursor:pointer; font-size:20px; width:30px; display:flex; justify-content:center;" aria-label="Condividi nozze">📤</button>
    <input type="text" value="${item.titolo_noz || 'NOZZE'}" 
           style="background:none; border:none; color:white; font-weight:900; text-align:center; flex:1; outline:none; font-family:inherit; font-size:18px;" 
           onblur="db.ref('agenda/${giornoCorrente}/${item.id}').update({titolo_noz:this.value})">
    <button onclick="eliminaRiga('${item.id}')" style="background:none; border:none; color:white; cursor:pointer; font-size:18px; width:30px; display:flex; justify-content:center;">🗑️</button>
</div>
        <div style="display:grid; gap:10px; padding:10px;">
            ${['cerimonia', 'ricevimento'].map(key => `
            ...
            <div class="admin-block" style="border-color:${GOLD};">
                <div class="admin-top-row">
                    <div class="admin-item">FOTO <input type="checkbox" ${item.foto?'checked':''} onchange="db.ref('agenda/${giornoCorrente}/${item.id}').update({foto:this.checked})"></div>
                    <textarea class="input-adm" rows="1" style="flex:1; min-width:120px;" placeholder="FOTOGRAFO" oninput="autoResize(this)" onblur="db.ref('agenda/${giornoCorrente}/${item.id}').update({op_foto:this.value})">${item.op_foto||''}</textarea>
                    <div class="admin-item">VIDEO <input type="checkbox" ${item.video?'checked':''} onchange="db.ref('agenda/${giornoCorrente}/${item.id}').update({video:this.checked})"></div>
...
<div class="titolo-battesimo">
    <!-- 1. Bottone Condividi a sinistra -->
    <button onclick="condividiSchema('${item.id}', 'matrimonio')" data-testid="btn-condividi-wed" aria-label="Condividi matrimonio"
            style="background:none; border:none; color:white; cursor:pointer; font-size:20px; width:30px; display:flex; align-items:center; justify-content:center; padding:0; margin:0;">📤</button>
    ...
    <div style="display:grid; gap:10px; padding:10px;">
        ...
        <div class="slot-main" style="background:white; padding:10px; border-radius:10px; border-left: 5px solid #1a237e;">
            <div class="ora-box" style="display:flex; flex-direction:column; gap:4px;">
                <input type="text" class="ora-input" placeholder="Inizio" title="Orario di Inizio" value="${item[k+'_h']||''}" onblur="db.ref('agenda/${giornoCorrente}/${item.id}').update({['${k}_h']:this.value})">
                <input type="text" class="ora-input" placeholder="Fine" title="Orario di Fine" value="${item[k+'_h_fine']||''}" onblur="db.ref('agenda/${giornoCorrente}/${item.id}').update({['${k}_h_fine']:this.value})">
            </div>
...
function aggiungiRigaExtra() { 
    ...
    db.ref(`agenda/${giornoCorrente}/${id}`).set({h:"", t:"", c:"def", sort:999}); 
    // Dopo il render, scrolla alla nuova riga e dai il focus all'orario
    // così l'utente la vede e può digitare subito l'orario.
    setTimeout(() => {
        const el = document.getElementById('slot-' + id);
        if (el) {
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            const oraInput = el.querySelector('.ora-input');
            if (oraInput) {
                oraInput.focus();
                // Quando l'utente digita l'orario e blura, ricalcola il sort
                // così la riga si riposiziona nell'ordine cronologico corretto.
                // Dopo il riordino, riportiamo la riga in viewport per evitare
                // che l'utente "perda" la riga appena modificata.
                oraInput.addEventListener('blur', () => {
                    const nuovaOra = oraInput.value.trim();
                    if (nuovaOra && nuovaOra !== '00:00' && nuovaOra !== '0:00') {
                        const nuovoSort = cleanH(nuovaOra) || 999;
                        db.ref(`agenda/${giornoCorrente}/${id}`).update({ h: nuovaOra, sort: nuovoSort })
                            .then(() => {
                                // Attendiamo il giro Firebase->listener->renderGiorno
                                // e poi riportiamo la nuova riga in viewport.
                                setTimeout(() => {
                                    const elDopo = document.getElementById('slot-' + id);
                                    if (elDopo) {
                                        elDopo.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                        elDopo.style.transition = 'background-color 0.6s';
                                        elDopo.style.backgroundColor = '#fff9c4';
                                        setTimeout(() => { elDopo.style.backgroundColor = ''; }, 1500);
                                    }
                                }, 350);
                            });
                    }
...
// === SCHEMA NOZZE ===
// Identico al Battesimo per campi, con flag isNozzeBlock e tonalità Oro/Bronzo (#b8860b).
function applicaSchemaNozze() {
    const ts = Date.now(); 
    db.ref('config/'+giornoCorrente).update({mostraOra:false, mostraRighe:false});
    ...
    db.ref('titoli/'+giornoCorrente).once('value', s => { 
        let tOld = s.val() || ""; 
        let tNew = tOld ? (tOld.includes("NOZZE") ? tOld : tOld + " E NOZZE") : "NOZZE"; 
        db.ref('titoli/'+giornoCorrente).set(tNew).then(() => {
            const ti = document.getElementById('titoloGiorno');
            if(ti) ti.value = tNew;
        }); 
    ...
    const id = "noz_" + ts; 
    const nozDataContent = { 
        isNozzeBlock: true, sort: 1, titolo_noz: "NOZZE", 
        cerimonia_h: "", cerimonia_t: "", cerimonia_c: "def", 
        ricevimento_h: "", ricevimento_t: "", ricevimento_c: "def", 
        note_t: "", note_c: "def", foto: false, op_foto: "", 
        video: false, op_video: "", acc1: "", dat1: "", chi1: "def" 
    };
    const nozData = { [id]: nozDataContent };

    db.ref(`agenda/${giornoCorrente}`).update(nozData).then(() => {
        if (!datiGiorno) datiGiorno = {};
        datiGiorno[id] = nozDataContent;
        const checkR = document.getElementById('checkRighe');
        if (checkR) checkR.checked = false;
        renderGiorno();
        console.log("Schema Nozze applicato");
    });

    db.ref('notifiche_log').push({ timestamp: Date.now(), dataGiorno: giornoCorrente, rigaId: id, oraRiga: "00:00", testo: "NOZZE INSERITO" });
    closeModal('mainModal');
}
...
let touchStripEl = null;
let touchStripScrollStart = 0;
...
let _scrolledDuringTouch = false;
let _scrollStartY = 0;

document.addEventListener('touchmove', e => {
    if (!e.touches || e.touches.length === 0) return;
    const y = e.touches[0].pageY;
    // Se la pagina si è scrollata durante il touch, marchiamolo per disabilitare lo swipe
    if (Math.abs(window.scrollY - _scrollStartY) > 5) _scrolledDuringTouch = true;
...
document.addEventListener('touchstart', e => { 
    ...
    _scrolledDuringTouch = false;
    _scrollStartY = window.scrollY;
    // Se il tocco parte DENTRO la striscia dei giorni, ci salviamo
    // lo scrollLeft iniziale: se l'utente arriva al bordo e continua a swipare,
    // cambiamo mese (vedi handleGesture).
    touchStripEl = (e.target && e.target.closest) ? e.target.closest('.calendar-strip') : null;
    touchTargetIsStrip = !!touchStripEl;
    touchStripScrollStart = touchStripEl ? touchStripEl.scrollLeft : 0;
}, { passive: true });
...
function handleGesture() { 
    ...
    // === CASO SPECIALE: SWIPE SULLO STRIP DEI GIORNI ===
    // Se l'utente arriva al bordo dello strip (scrollLeft = 0 o = max) e
    // continua a swipare nella stessa direzione, cambiamo mese.
    if (touchTargetIsStrip && touchStripEl) {
        const stripSogliaX = 50; // soglia bassa: lo scroll dello strip "assorbe" il gesto
        const orizzontale = Math.abs(dx) > stripSogliaX && Math.abs(dx) > Math.abs(dy) * 1.3;
        if (!orizzontale) { touchStripEl = null; return; }
        
        const maxScroll = touchStripEl.scrollWidth - touchStripEl.clientWidth;
        // Consideriamo "al bordo" se lo strip non si è praticamente mosso E
        // siamo già attaccati al bordo iniziale o finale.
        const restatoFermo = Math.abs(touchStripEl.scrollLeft - touchStripScrollStart) < 8;
        const alBordoDestro = touchStripEl.scrollLeft >= maxScroll - 4;
        const alBordoSinistro = touchStripEl.scrollLeft <= 4;
        
        if (dx < 0 && alBordoDestro && restatoFermo) {
            // Swipe a sinistra → mese successivo, e seleziona il primo giorno
            cambiaMeseOffset(1);
            setTimeout(() => {
                const newStrip = document.getElementById('strip');
                if (newStrip) newStrip.scrollLeft = 0;
                _selezionaPrimoGiornoMese();
            }, 80);
        } else if (dx > 0 && alBordoSinistro && restatoFermo) {
            // Swipe a destra → mese precedente, e seleziona l'ultimo giorno
            cambiaMeseOffset(-1);
            setTimeout(() => {
                const newStrip = document.getElementById('strip');
                if (newStrip) newStrip.scrollLeft = newStrip.scrollWidth;
                _selezionaUltimoGiornoMese();
            }, 80);
        }
        touchStripEl = null;
        return;
    ...
    // 1. Se la pagina è stata scrollata durante il touch → era uno scroll, non uno swipe
    if (_scrolledDuringTouch) return;
    
    const sogliaX = 120; // alzata da 80 a 120 px per evitare swipe accidentali
    
    // 2. Movimento verticale prevalente → scroll, non swipe
    if (Math.abs(dy) > Math.abs(dx)) return;
    
    // 3. Richiediamo che il movimento X sia almeno 1.8x il movimento Y (swipe veramente orizzontale)
    if (Math.abs(dx) < Math.abs(dy) * 1.8) return;
    
    // 4. Soglia X minima rigorosa
...
// Helper: dopo cambio mese, seleziona il primo giorno del nuovo mese
function _selezionaPrimoGiornoMese() {
    const mp = document.getElementById('monthPicker');
    if (!mp || !mp.value) return;
    const iso = mp.value + '-01';
    selezionaGiorno(iso, true);
}

// Helper: dopo cambio mese, seleziona l'ultimo giorno del nuovo mese
function _selezionaUltimoGiornoMese() {
    const mp = document.getElementById('monthPicker');
    if (!mp || !mp.value) return;
    const [y, m] = mp.value.split('-').map(Number);
    const ultimoGiorno = new Date(y, m, 0).getDate();
    const iso = `${y}-${String(m).padStart(2,'0')}-${String(ultimoGiorno).padStart(2,'0')}`;
    selezionaGiorno(iso, true);
}
...
function saltaAggiOggi() {
    ...
    // 1. Riporta sempre la pagina in cima per far vedere l'header e la striscia dei giorni
    window.scrollTo({ top: 0, behavior: 'smooth' });
    ...
    if (mp.value !== meseOggi) {
        ...
        toggleVista('g');
        selezionaGiorno(isoOggi, true);
        // Secondo scroll, dopo l'eventuale animazione dello strip, per assicurarci
        // che il giorno odierno sia ben visibile nell'header (non coperto da altro)
...
const CONFIG_ST = [
    ...
    { label: 'Nozze',             color: '#b8860b', keys: ['nozze'],                              blockFlag: 'isNozzeBlock' },
...
function eliminaRiga(id) {
    ...
    if (confirm("Vuoi eliminare definitivamente questo blocco e il relativo titolo?")) {
        ...
        const itemCached = (datiGiorno && datiGiorno[id]) ? { ...datiGiorno[id] } : null;
        const descCancellato = itemCached
            ? (itemCached.titolo_bat || itemCached.titolo_noz || itemCached.titolo_wed || itemCached.t || 'voce')
            : 'voce';
        const oraCancellata = itemCached ? (itemCached.h || '') : '';
...
// produce molte varianti: "sotto mano" (2 parole), "sotto la mano", "sotto-mano",
// "sotto mando" (errore di riconoscimento comune), ecc.
function _haKeywordSottomano(lc) {
    return /\bsottomano\b|sotto[\s\-]+(?:la\s+|al\s+|il\s+)?man[od]\b|promemoria|appuntalo|\bappunto\b|\bappunti\b|\bmemo\b|\bnota\s+(?:rapida|veloce|in\s+sottomano)\b/i.test(lc);
}
function _haKeywordSpesa(lc) {
    return /\blista\s+(?:della\s+|di\s+)?spesa\b|\bspesa\b|\bspese\b|\bfare\s+la\s+spesa\b/i.test(lc);
}
...
function _cancellaImpegnoAgenda(ora, testoQuery, giornoTarget) {
    ...
    db.ref(`agenda/${giornoG}`).once('value', snap => {
        ...
        for (const [id, item] of Object.entries(data)) {
            ...
            if (item.isWedBlock || item.isBattesimoBlock || item.isNozzeBlock) continue;
...
function elaboraComandoVocale(testo) {
    ...
    const nozEntry = Object.entries(datiGiorno || {}).find(([id, it]) => it && it.isNozzeBlock);
    ...
    if (nozEntry && _gestisciSchemaBattesimo(nozEntry[0], lcOriginale, oraInfo.ora, telInfo.telefono, testoBase)) {
        // Lo schema NOZZE ha gli stessi campi del Battesimo (cerimonia/ricevimento/note),
        // quindi riusiamo lo stesso handler.
        return;
    }
...
function _evidenziaRiga(id) {
    // Provo più volte perché il render potrebbe non essere ancora completato
    let tentativi = 0;
    const cerca = () => {
        const el = document.getElementById('slot-' + id);
        if (!el) {
            tentativi++;
            if (tentativi < 6) {
                setTimeout(cerca, 200);
            }
            return;
        }
        // Scroll: usiamo "start" così la riga appare in alto, ben visibile anche
        // se sotto a un blocco matrimonio molto alto.
        el.scrollIntoView({ behavior: 'smooth', block: 'start' });
        // Compensazione per l'header fisso (~210px in alto)
        setTimeout(() => window.scrollBy(0, -180), 350);
        const oldBg = el.style.backgroundColor;
        el.style.transition = 'background-color 0.4s';
        el.style.backgroundColor = '#fff9c4';
        el.style.boxShadow = '0 0 0 3px #fbc02d';
        setTimeout(() => { 
...
// =============================================================================
// CONDIVISIONE SCHEMA (Matrimonio / Battesimo) — solo dati pubblici
// Esclude qualunque informazione amministrativa interna: acconti, operatore,
// fotografo, video, flag foto/video, chi (ric/a/d) per gli acconti, ecc.
// =============================================================================
function _formattaSchemaPerCondivisione(item, tipo) {
    if (!item) return '';
    const dataLeggibile = giornoCorrente
        ? new Date(giornoCorrente).toLocaleDateString('it-IT', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })
        : '';
    const lines = [];
    const aggiungi = (etichetta, h, t) => {
        const oh = (h && h !== 'undefined' && h !== '00:00' && h !== '0:00') ? h : '';
        const ot = (t || '').toString().trim();
        if (!oh && !ot) return;
        // Rimuoviamo eventuali ridondanze tipo "SPOSO: SPOSO: Mario"
        const otClean = ot.replace(new RegExp('^' + etichetta.toUpperCase() + ':\\s*', 'i'), '').trim();
        if (oh && otClean) lines.push(`*${etichetta}* — ${oh}\n${otClean}`);
        else if (oh)       lines.push(`*${etichetta}* — ${oh}`);
        else               lines.push(`*${etichetta}*\n${otClean}`);
    };
    
    if (tipo === 'matrimonio') {
        const titolo = (item.titolo_wed || 'MATRIMONIO').toUpperCase();
        lines.push(`💒 *${titolo}*`);
        if (dataLeggibile) lines.push(`📅 ${dataLeggibile}`);
        lines.push('');
        
        ['sposo','sposa'].forEach(k => {
            const t  = item[k + '_t'];
            const tl = item[k + '_tel'];
            const via= item[k + '_via'];
            const h  = item[k + '_h'];
            const hFine = item[k + '_h_fine'];
            if (!t && !tl && !via && !h && !hFine) return;
            const label = k.toUpperCase();
            const parts = [];
            const hValido = h && h !== '00:00' && h !== '0:00';
            const hFineValido = hFine && hFine !== '00:00' && hFine !== '0:00';
            if (hValido && hFineValido) parts.push(`⏰ ${h} - ${hFine}`);
            else if (hValido) parts.push(`⏰ ${h}`);
            else if (hFineValido) parts.push(`⏰ → ${hFine}`);
            const tNoPrefix = (t || '').replace(new RegExp('^' + label + ':\\s*', 'i'), '').trim();
            if (tNoPrefix) parts.push(`👤 ${tNoPrefix}`);
            const telNoPrefix = (tl || '').replace(/^TEL:\s*/i, '').trim();
            if (telNoPrefix) parts.push(`📞 ${telNoPrefix}`);
            const viaNoPrefix = (via || '').replace(/^VIA:\s*/i, '').trim();
            if (viaNoPrefix) parts.push(`📍 ${viaNoPrefix}`);
            if (parts.length) {
                lines.push(`*${label}*`);
        ...
        ['chiesa','sala'].forEach(k => {
            const t = item[k + '_t'];
            const h = item[k + '_h'];
            aggiungi(k.toUpperCase(), h, t);
        });
        
        // Esterni: max 5 location aggiuntive
        const esterni = [];
        for (let i = 1; i <= 5; i++) {
            const h = item['loc_h' + i];
            const t = item['loc_t' + i];
            if ((h && h !== '00:00' && h !== '0:00') || (t && t.trim())) {
                esterni.push([h, t].filter(Boolean).join(' — '));
            }
        }
        if (esterni.length) {
            lines.push('');
            lines.push('*🚗 ESTERNI*');
            esterni.forEach(e => lines.push(`• ${e}`));
        }
        
        if (item.note_t && item.note_t.trim()) {
            lines.push('');
            const noteNoPrefix = item.note_t.replace(/^NOTE:\s*/i, '').trim();
            if (noteNoPrefix) {
                lines.push('*📝 NOTE*');
                lines.push(noteNoPrefix);
            }
        }
    } else if (tipo === 'battesimo') {
        const titolo = (item.titolo_bat || 'BATTESIMO').toUpperCase();
        lines.push(`👶 *${titolo}*`);
        if (dataLeggibile) lines.push(`📅 ${dataLeggibile}`);
        lines.push('');
        
        ['cerimonia','ricevimento'].forEach(k => {
            aggiungi(k.toUpperCase(), item[k + '_h'], item[k + '_t']);
        });
        ...
        const titolo = (item.titolo_noz || 'NOZZE').toUpperCase();
        lines.push(`💛 *${titolo}*`);
        if (dataLeggibile) lines.push(`📅 ${dataLeggibile}`);
        lines.push('');
...
async function condividiSchema(itemId, tipo) {
    const item = datiGiorno && datiGiorno[itemId];
    if (!item) { mostraToast("Impossibile leggere i dati"); return; }
    
    const testo = _formattaSchemaPerCondivisione(item, tipo);
    if (!testo) { mostraToast("Nessun dato da condividere"); return; }
    
    const titleMap = { matrimonio: 'Matrimonio', battesimo: 'Battesimo', nozze: 'Nozze' };
    
    // Web Share API nativa (mobile moderni) — apre la dialog di sistema
    if (navigator.share) {
        try {
            await navigator.share({
                title: titleMap[tipo] || 'Schema',
                text: testo
            });
            return;
        } catch (e) {
            // L'utente ha annullato o non supportato → fallback
            if (e && e.name === 'AbortError') return; // ha chiuso il dialog
        }
    }
    
    // Fallback: apre WhatsApp con il testo precompilato
