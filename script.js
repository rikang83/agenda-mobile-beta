...
function inviaPromemoriaRiga(id) {
    const riga = datiGiorno[id];
    if (!riga) return;

    const testo = riga.t || "";
    const ora = riga.h || "00:00"; 

    // Estrazione numero di telefono dal testo della riga
    const matchTel = testo.match(/\d{8,}/); 
    let tel = matchTel ? matchTel[0] : "";

    if (!tel) {
        tel = prompt("Inserisci il numero del cliente (senza prefisso):", "");
    }
    if (!tel) return;

    // Pulizia e formattazione numero
    tel = tel.replace(/\D/g, '');
    if (!tel.startsWith('39') && tel.length < 11) tel = '39' + tel;

    // Generazione Link Calendario Google
    const d = giornoCorrente.split('-'); 
    const dataCal = d[0] + d[1] + d[2];
    const oraCal = ora.replace(':', '') + "00";
    const linkCal = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(testo)}&dates=${dataCal}T${oraCal}/${dataCal}T${oraCal}`;

    const msg = `Gentile cliente, le confermiamo l'appuntamento:\n\n📅 *${giornoCorrente}*\n🕒 ore *${ora}*\n📍 ${testo}\n\nSalva l'appuntamento in calendario cliccando qui: ${linkCal}`;
