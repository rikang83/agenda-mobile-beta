...
if (checkOra) {
    checkOra.checked = conf.mostraOra !== false;
}
...
div.innerHTML = `
    <div style="display:flex; flex-direction:column; width:100%;">
        ...
        <div style="display:flex; align-items:flex-start; width:100%; gap:10px;">
            <div class="ora-box">
                <input type="text" class="ora-input" placeholder="00:00" value="${item.h || ''}" 
            ...
            <input type="text" class="nota-input" style="flex:1;" value="${item.t || ''}" 
                   onblur="db.ref('agenda/${giornoCorrente}/${item.id}').update({t:this.value})">
        </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-top:10px; width:100%;">
            ...
            <div class="color-dots" style="display:flex; align-items:center; gap:8px;">
                ${(!item.isWed && !item.isAdmin) ? Object.keys(colMap).filter(k=>k!='def').map(k=>`<div class="dot ${item.c===k?'active':''}" style="background:${colMap[k][0]}" onclick="cambiaColore('${item.id}','${k}','${item.h}')">${colMap[k][1]}</div>`).join('') : ''}
...
// Funzione per inserire il tasto senza rompere l'HTML esistente
function inserisciTastoSottomano() {
    const masterSwitch = document.querySelector('.master-switch');
    if (masterSwitch && !document.getElementById('btn-sottomano-auto')) {
        const btn = document.createElement('button');
        btn.id = 'btn-sottomano-auto';
        btn.onclick = openSottomano;
        btn.className = 'nav-btn';
        btn.innerHTML = '📝 SOTTOMANO';
        btn.style.cssText = "background: #fffbe6; color: #FF9800; border: 1.5px solid #ffe082; height: 38px; padding: 0 10px; font-weight: 900; font-size: 11px; margin-left: 8px;";
        
        // Lo inserisce subito dopo lo switch "R"
        masterSwitch.parentNode.insertBefore(btn, masterSwitch.nextSibling);
    }
}
...
// Controllo ogni secondo per assicurarci che non sparisca
setInterval(inserisciTastiHeader, 1000);
// Controllo continuo per evitare che l'agenda lo cancelli
setInterval(inserisciTastiHeader, 1000);
