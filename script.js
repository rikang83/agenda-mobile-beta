...
function handleGesture() { 
    ...
    // 2. Se la pagina è stata scrollata durante il touch → era uno scroll, non uno swipe
    ...
    // 3. Movimento verticale prevalente → scroll, non swipe
    if (Math.abs(dy) > Math.abs(dx)) return;
    
    // 4. Richiediamo che il movimento X sia almeno 1.8x il movimento Y (swipe veramente orizzontale)
    if (Math.abs(dx) < Math.abs(dy) * 1.8) return;
    
    // 5. Soglia X minima rigorosa
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: tipo === 'matrimonio' ? 'Matrimonio' : 'Battesimo',
                text: testo
            });
            return;
        } catch (e) {
