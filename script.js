...
function avviaAssistenteVocale() {
    ...
    // Per sottomano/lista spesa non serve un giorno selezionato; per agenda sì
    const contesto = _detectContestoVocale();
    if (contesto === 'agenda' && !giornoCorrente) {
...
function _aggiungiVoceLista(pathDB, testo, etichetta) {
    const id = Date.now();
    db.ref(`${pathDB}/${id}`).set({ id: id, t: testo, fatto: false });
    creaNotifica(`${pathDB}-${id}`, '', `✓ Aggiunto in ${etichetta}: ${testo.substring(0, 50)}`);
...
function _cancellaVoceLista(pathDB, testoDaCercare, etichetta) {
    ...
    db.ref(pathDB).once('value', snap => {
        ...
        if (rimossi > 0) {
            creaNotifica(`${pathDB}-pulizia`, '', `🗑️ Rimosso/i ${rimossi} da ${etichetta}: ${testoDaCercare}`);
...
function _cancellaImpegnoAgenda(ora, testoQuery) {
    if (!giornoCorrente || !datiGiorno) { mostraToast("Niente da cancellare"); return; }
    const queryL = (testoQuery || '').toLowerCase().trim();
    let candidato = null;
    
    for (const [id, item] of Object.entries(datiGiorno)) {
        if (!item || typeof item !== 'object') continue;
        if (item.isWedBlock || item.isBattesimoBlock) continue; // gli schemi si cancellano col cestino, non a voce
    ...
    db.ref(`agenda/${giornoCorrente}/${candidato.id}`).remove();
    if (datiGiorno && datiGiorno[candidato.id]) delete datiGiorno[candidato.id];
    creaNotifica(candidato.id, candidato.item.h || '', `🗑️ Cancellato: ${String(descr).substring(0, 50)}`);
...
function elaboraComandoVocale(testo) {
    ...
    if (matchCancella) {
        ...
        if (contesto === 'sottomano') {
            _cancellaVoceLista('sottomano', rest, 'SOTTOMANO');
        } else if (contesto === 'spesa') {
            _cancellaVoceLista('lista_spesa', rest, 'LISTA SPESA');
        } else {
            if (!giornoCorrente) { mostraToast("Seleziona prima un giorno."); return; }
            const oraInfo = _estraiOrario(rest);
            const testoQuery = _pulisciTesto(oraInfo.restante);
            _cancellaImpegnoAgenda(oraInfo.ora, testoQuery);
        }
        return;
    }
    
    // 2) CONTESTO SOTTOMANO / SPESA: aggiunta semplice (no parsing orario)
    if (contesto === 'sottomano') {
    ...
    // 3a) Estrazione orario e telefono
    ...
    // 3b) Schemi attivi nel giorno corrente
    ...
    // 3d) Riga normale: salva nello slot orario corrispondente o crea nuova riga
