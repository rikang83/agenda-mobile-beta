...
if(item.isBattesimoBlock) {
    ...
    <button onclick="del('${item.id}')" style="background:none; border:none; color:white; cursor:pointer; font-size:18px; width:30px; display:flex; justify-content:center;">🗑️</button>
</div>
...
function openStatsModal() {
    // 1. Chiude il menu delle impostazioni (mainModal)
    closeModal('mainModal');
    
    // 2. Apre il modale del grafico (che nel tuo HTML si chiama chartModal)
    openModal('chartModal');
    
    // 3. Avvia il caricamento dei dati (se la funzione esiste)
    if (typeof aggiornaGrafici === "function") {
        aggiornaGrafici();
    } else if (typeof caricaDatiGrafico === "function") {
        caricaDatiGrafico();
    }
}

function aggiornaGrafici() {
    const dbRef = firebase.database().ref('impegni');
    dbRef.once('value', (snapshot) => {
        const tuttiDati = snapshot.val() || {};

        const categories = [
            { label: 'Matrimoni', color: '#1a237e', keys: ['matrimonio'] },
            { label: 'Battesimi', color: '#64b5f6', keys: ['battesimo'] },
        ...
        let stats = categories.map(() => new Array(12).fill(0));
        let totaleGlobale = 0;

        Object.keys(tuttiDati).forEach(dataChiave => {
            // Rilevamento flessibile del mese
            let meseIndex = -1;
            const parti = dataChiave.split('-');
            
            if (parti.length === 3) {
                // Se la data è YYYY-MM-DD, il mese è in posizione 1
                // Se la data è DD-MM-YYYY, il mese è in posizione 1
                meseIndex = parseInt(parti[1]) - 1;
            }

            if (meseIndex >= 0 && meseIndex < 12) {
                const impegniGiorno = tuttiDati[dataChiave];
                Object.values(impegniGiorno).forEach(imp => {
                    if (imp && imp.testo) {
                        const testoMinuscolo = imp.testo.toLowerCase(); // Trasformazione in minuscolo
                        categories.forEach((cat, i) => {
                            if (cat.keys.some(key => testoMinuscolo.includes(key))) {
                                stats[i][meseIndex]++;
                                totaleGlobale++;
                            }
                        });
        ...
        window.myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: categories.map((cat, i) => ({
                    ...
                    borderColor: cat.color,
                    backgroundColor: cat.color,
                    tension: 0.3, // Curve come da foto
                    fill: false,
                    pointRadius: 5
                }))
            },
