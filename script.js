...
function salvaCampo(id, campo, valore, oraDef, isSub=false) { 
    ...
    if (campo === 't' && valore.trim().length > 1 && !isSub) { 
        ...
        // --- INVIO NOTIFICA "SICURO" ---
        // Usiamo window['Capacitor'] così il browser non crasha se non lo trova
        const cap = window['Capacitor'];
        if (cap && cap.Plugins && cap.Plugins.LocalNotifications) {
            cap.Plugins.LocalNotifications.schedule({
                notifications: [{
                    title: "Modifica Agenda",
                    body: "Ora: " + (oraInput || '00:00') + " - " + testoSoggetto,
                    id: Math.floor(Math.random() * 1000),
                    schedule: { at: new Date(Date.now() + 1000) }
                }]
            }).catch(err => console.log("Errore notifica ignorato"));
        }

        // --- IL TUO LOG FIREBASE ORIGINALE ---
